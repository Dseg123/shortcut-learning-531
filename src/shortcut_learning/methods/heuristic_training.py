...
class GoalConditionedWrapper(gym.Wrapper):
    """Wrapper to make any gym environment goal-conditioned with -1 step reward."""
...
def heuristic_training(env: gym.Env, 
                        heuristic_model: HeuristicModel, 
                        shortcut_pairs: list[tuple[PlanningGraphNode, PlanningGraphNode]]) -> HeuristicModel:
    """ Given ShortcutTrainingData and a heuristic model, run an RL algorithm over each shortcut pair 
    to determine its path length. We will use this path length to train our heuristic model.
...
def _calculate_path_length_heuristic(env: gym.Env, shortcut_pair: tuple[PlanningGraphNode, PlanningGraphNode]) -> float:
    """Given an PlanningGraphNode pair, run rollouts with RL across corresponding low-level states
    to determine the plan length heuristic. 
